...
type famille_op = float -> float -> (float -> float -> float) -> float
...
let binop = SS2.of_list ["+";"-";"/";"*";"POW"] 
let unop = SS2.of_list ["NEG";"SIN";"COS";"TAN";"COSH";"SINH";"TANH";"ARCSIN";"ARCOS";"ARCTAN";"EXP";"LN";"LOG10";"SQRT"] 
...
let parse (s : string) = 
    let rec p (s2 : string list) (acc : mix list) : mix list  =
        ...
        | x::xs when SS2.mem x binop -> (Operateur (Binaire (assoc_bin x))) :: acc |> p xs
        | x::xs when SS2.mem x unop -> ((Operateur (Unaire (assoc_un x))) :: acc) |> p xs 
